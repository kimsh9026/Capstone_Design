  ...
  void search() {
    if(_searchAddr != ''){
      FocusScope.of(_context).detach() ;
        ...
        isLoading = true ;
        print('loading') ;
        BlocProvider.of(context).mapBloc.setSearchingOrLoading(true) ;
        _placeList = List<PlacesSearchResult>() ;
...
//    BlocProvider.of(_context).mapBloc.setLocation(result.vicinity) ;
  ...
  Widget _expandingSearchBar(){
    return StreamBuilder(
      stream: BlocProvider.of(context).mapBloc.searchingOrLoading,
  ...
  getUserLocation() async {
    ...
    _places.searchNearbyWithRadius(Location(_currentLocation.latitude, _currentLocation.longitude), 2500)
      ...
      temp['name'] = value.results[0].name ;
      temp['center'] = LatLng(0,0) ;
      BlocProvider.of(_context).mapBloc.setLocation(temp) ;
    });
    setState(() {
...
//    _mapOffSubscription = BlocProvider.of(_context).mapBloc.addRoomMapOff.listen((value){
//      if(value){
//        if(isSearching){
//          print('isSearching?') ;
//          isSearching = false ;
          ...
          onTap: (){
            if(_center != LatLng(0,0)){
              ...
              Navigator.push(context, MaterialPageRoute(
                  builder: (context){
                    return FullScreenMapApi(_markers, _center) ;
                  }
              ));
            }

//            BlocProvider.of(context).mapBloc.setAddRoomMapOff(false) ;
          },
        ),
...
//                  color: Colors.white
//              ),
//              child: isOn ? _expandingSearchBar() : Container(),
//            )
//        )
//      ],
//    );
